...
export default async function SquadPage({searchParams}:{searchParams:Promise<{team?:string}>}){
  ...
  return <div className="content max">
    <div className="page-kicker">COLLEGIATE VENDAS // EXECUTIVOS CADASTRADOS</div>
    <h1 className="page-title">Esquadrão</h1>
    <p className="page-sub">Os cards usam exatamente o mesmo avatar, background, ranking, VGV e visual da tela inicial. Somente vendas aprovadas entram no desempenho.</p>

    <div className="section-head"><h2>Squads da <b>Operação</b></h2><div className="line"/><div className="meta">filtre os executivos</div></div>
    ...
    <div className="squad-grid compact-squad">
      {filtered.map((op:any,index:number)=><OperatorCard key={op.id} op={op} index={index} leader={index===0}/>)}
    </div>
