...
export default function AppShell({children,viewer}:{children:React.ReactNode;viewer:{fullName:string;role:string}}){
  ...
  return <div className="app-shell">
    <aside className="sidebar">
      <div className="brand-wrap"><Image className="side-logo" src="/logo-collegiate-white.png" width={1257} height={635} alt="Collegiate Vendas" priority/></div>
    ...
    <main className="main">
      <header className="topbar"><div className="top-title"><div className="op-mark">Operação <b>Sexta Marcha</b></div><div className="top-meta">disciplina · estratégia · execução</div></div><div className="viewer-pill"><div className="viewer-avatar">{initials}</div><div><strong>{viewer.fullName}</strong><small>{viewer.role}</small></div></div></header>
      {children}
    </main>
  </div>
}
