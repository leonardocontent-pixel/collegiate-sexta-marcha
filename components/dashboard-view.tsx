...
export default function DashboardView({ data }: { data: any }) {
  ...
  const goals = normalizeGoalTiers(data.goals?.length ? data.goals : DEFAULT_GOAL_TIERS)
  const target = Number(campaign.target_vgv || goals[goals.length - 1]?.threshold_vgv || 7000000)
  const confirmed = Number(summary.confirmed_vgv || 0)
  const progress = Math.min(100, target ? confirmed / target * 100 : 0)
  const operationDone = confirmed >= target && target > 0
  ...
  return <div className="content max warroom-dashboard page-enter">
    ...
    <div className="section-head reveal reveal-3"><h2>Metas e <b>Insígnias</b></h2><div className="line" /><div className="meta">um objetivo · quatro condecorações</div></div>
    <div className="badge-level-grid reveal reveal-3">{goals.map((level, index) => {
      const done = confirmed >= level.threshold_vgv
      const current = !done && (index === 0 || confirmed >= goals[index - 1].threshold_vgv)
      const tierProgress = Math.max(0, Math.min(100, level.threshold_vgv ? (confirmed / level.threshold_vgv) * 100 : 0))
      return <div className={`badge-level ${level.tone} ${done ? 'done' : ''} ${current ? 'current' : ''}`} key={`${level.name}-${index}`}>
        {done && <div className="objective-seal">Objetivo concluído</div>}
        <div className="insignia"><img src={level.badge} alt={`Insígnia ${level.name}`} /></div>
        <div className="badge-level-copy">
          <div className="level-name">{level.name}</div>
          <div className="level-reward">{brl(level.threshold_vgv)} · {level.reward}</div>
          ...
          <div className="level-progress-wrap">
            <div className="level-progress-head"><span>Avanço da meta</span><span>{pct(tierProgress)}</span></div>
            <div className="level-progress-track"><div className={`level-progress-fill ${done ? 'done' : ''}`} style={{ '--tier-progress': `${tierProgress}%` } as CSSProperties} /></div>
          </div>
        </div>
      </div>
    })}</div>
